...
public class ExContextFactory
{
  ...
  private void refreshContexts() throws Exception
  {
    ...
    try
    {
      ...
      if (rb.containsKey("useSSL"))
      {
        boolean useSSL = Boolean.parseBoolean(rb.getString("useSSL"));
        props.setProperty(MpiNetRR.USE_SSL, String.valueOf(useSSL));
        if (useSSL)
        {
          info("SSL is used.");
        } else
      ...
      {
        info("SSL defaults to WSDL settings");
      }
